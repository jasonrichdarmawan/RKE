...
def calculate_metrics(data):
    ...
    for index in tqdm(range(len(data)), desc="Calculate BLEU&ROUGE Score"):
        score = sentence_bleu(
            [data[index]["answer"]], data[index]["original_prediction"]
        )
    ...
    embeddings1 = model.encode(
        sentences1, convert_to_tensor=True, show_progress_bar=True
    )
    embeddings2 = model.encode(
        sentences2, convert_to_tensor=True, show_progress_bar=True
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--file_path", default="output/unke_Llama3-8B-Instruct_cf_result.json", type=str
    )
    parser.add_argument(
        "--model_path", default="sentence-transformers/all-MiniLM-L6-v2", type=str
    )
    ...
    if ds_name == "editevery":
        ...
        for index in tqdm(range(len(data)), desc="Calculate BLEU&ROUGE Score"):
            ...
            if ds_name in ["unke", "cf"]:
                scores = rouge.get_scores(
                    data[index]["para_prediction"], data[index]["answer"]
                )
            ...
            for i in range(len(data[index]["sub_pred"])):
                scores = rouge.get_scores(
                    data[index]["sub_pred"][i], data[index]["sub_answer"][i]
                )
